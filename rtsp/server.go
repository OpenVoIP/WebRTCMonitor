...
func StartRTSPServer() {
	url := "rtsp://192.168.12.132:554/user=admin&password=&channel=2&stream=0.sdp"
	sps := []byte{}
	pps := []byte{}
	fuBuffer := []byte{}
	count := 0
	...
	writeNALU := func(sync bool, ts int, payload []byte) {
		...
		// 数据写入 web 端
		if web.VideoTrack != nil && preTS != 0 {
			web.VideoTrack.WriteSample(media.Sample{Data: payload, Samples: uint32(ts - preTS)})
		}
		preTS = ts
	}
